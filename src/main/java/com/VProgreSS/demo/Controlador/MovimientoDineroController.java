...
import com.VProgreSS.demo.Servicios.ServicioIMovimientoDinero;
...
public class MovimientoDineroController {
    ...
    @Autowired
    ServicioIMovimientoDinero servicioIMovimientoDinero;
    ...
    public ResponseEntity<List<MovimientoDinero>> getTransaction(){
        return new ResponseEntity<>(servicioIMovimientoDinero.getTransaction(), HttpStatus.ACCEPTED);
    }

    //Mapping para traer una sola Transaction
    @GetMapping("/Movimiento/{idTransaction}")
    public ResponseEntity<Object> getTransactionPath(@PathVariable Long idTransaction){
        try {
            MovimientoDinero movimientoDineroX = servicioIMovimientoDinero.getOnlyOneTransaction(idTransaction);
    ...
    public ResponseEntity<String> PostCreateTransaction(@RequestBody MovimientoDinero movimientoDineroX){
        try {
            String message = servicioIMovimientoDinero.getCreateTransaction(movimientoDineroX);
    ...
    public ResponseEntity<ObjetivoVpro> putUpdateTransaction(@RequestBody MovimientoDinero movimientoDineroX){
        try {
            MovimientoDinero movimientoDineroBD = servicioIMovimientoDinero.getUpdateTransaction(movimientoDineroX);
            return new ResponseEntity<>(new ObjetivoVpro("Actualizacion de Transaction Exitosa", movimientoDineroBD),HttpStatus.OK);
        }catch (Exception e){
    ...
    @DeleteMapping("/EliminarMovimiento/{idTransaction}")
    public ResponseEntity<String> DeleteTransaction(@PathVariable Long idTransaction){
        try {
            String message = servicioIMovimientoDinero.getDeleteTransaction(idTransaction);
            return new ResponseEntity<>(message,HttpStatus.OK);
        }catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(),HttpStatus.INTERNAL_SERVER_ERROR);
        }
